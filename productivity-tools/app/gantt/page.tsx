...
// import { useGanttSync } from '@/hooks/use-gantt-sync'; // オフライン仕様のため無効化
...
function GanttPageContent() {
  ...
  // リアルタイム同期（オフライン仕様のため無効化）
  // const { sendTaskUpdate, sendDependencyUpdate } = useGanttSync({
  //   projectId: 'default',
  //   onTaskUpdate: (task) => {
  //     updateTask(task);
  //   },
      ...
      <div className="flex-1 flex">
        <div className={showCriticalPath ? 'flex-1' : 'w-full'}>
          <GanttChart
            ...
            onTaskUpdate={(task) => {
              updateTask(task);
              // sendTaskUpdate(task); // オフライン仕様のため無効化
            }}
            onTaskClick={(task) => {
